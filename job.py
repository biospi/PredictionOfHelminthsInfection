...
def famacha_dataset_exists(path):
    folders = [
        ...
        and "temp" not in x[0]
        and "hum" not in x[0]
        and "temp_humidity" not in x[0]
    ]
    return (
        len(folders) > 0,
        [x for x in folders if "gain" in x and "entireday" in x][::-1],
        [x for x in folders if "li" in x and "entireday" in x][::-1],
        [x for x in folders if "gain" in x and "nightonly" in x][::-1],
        [x for x in folders if "li" in x and "nightonly" in x][::-1],
    )
...
def weather_files_exist(path):
    temperature_files = [
        glob.glob(x[0] + "/*.csv")[0]
        for x in os.walk(path)
        if len(x) > 0 and "temp" in x[0] and "temp_humidity" not in x[0]
    ]
    humidity_files = [
        glob.glob(x[0] + "/*.csv")[0]
        for x in os.walk(path)
        if len(x) > 0 and "hum" in x[0] and "temp_humidity" not in x[0]
    ]
    return (
        len(temperature_files) > 0 and len(humidity_files) > 0,
        temperature_files[::-1],
        humidity_files[::-1],
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--batch_size",
        help="for imputation, the number of samples in mini-batch",
        default=128,
        type=int,
    )
    parser.add_argument(
        "--hint_rate", help="for imputation, hint probability", default=0.9, type=float
    )
    parser.add_argument(
        "--alpha", help="for imputation, hyperparameter", default=100, type=float
    )
    parser.add_argument(
        "--iterations",
        help="for imputation, number of training interations",
        default=600,
        type=int,
    )
    parser.add_argument(
        "--miss_rate",
        help="for imputation, missing data probability",
        default=0.0,
        type=float,
    )
    ...
    parser.add_argument(
        "--enable_anscombe",
        help="for imputation, appy anscombe on activity count before imputation",
        type=bool,
        default=False,
    )
    parser.add_argument(
        "--enable_remove_zeros",
        help="for imputation, remove zero counts in activity before imputation",
        type=bool,
        default=False,
    )
    parser.add_argument(
        "--enable_log_anscombe",
        help="for imputation, appy log(anscombe) on activity count before imputation",
        type=bool,
        default=True,
    )
    ...
    parser.add_argument(
        "--export_csv",
        type=bool,
        help="for imputation, export imputed traces as csv",
        default=True,
    )
    ...
    parser.add_argument(
        "--reshape",
        type=str,
        help="for imputation, reshape activity traces to 1 day chunck",
        default="y",
    )
    parser.add_argument("--w", type=str, default="n")
    parser.add_argument(
        "--add_t_col",
        help="for imputation, add time column in reshape",
        type=str,
        default="y",
    )
    parser.add_argument(
        "--thresh_daytime",
        help="for imputation, minimum number of positive count in 1 day.",
        default=100,
        type=int,
    )
    parser.add_argument(
        "--thresh_nan_ratio",
        help="for imputation, max percent of nan allowed in 1 day.",
        default=80,
        type=int,
    )

    parser.add_argument(
        "--ndays", help="Number of days in samples", default=7, type=int
    )
    ...
    ) = famacha_dataset_exists(args.output_dir)
    ...
    ml.main(
        steps_night,
        args.output_dir + "/ml/ml_kfold_2to2_7day_night",
        dataset_files_gain_night[0],
        "1To1",
        "2To2",
        False,
        12,
    ...
    ml.main(
        steps_night,
        args.output_dir + "/ml/ml_kfold_2to2_6day_night",
        dataset_files_gain_night[1],
        "1To1",
        "2To2",
        False,
        12,
    ...
    ml.main(
        steps_night,
        args.output_dir + "/ml/ml_kfold_2to2_5day_night",
        dataset_files_gain_night[2],
        "1To1",
        "2To2",
        False,
        12,
    ...
    ml.main(
        steps_night,
        args.output_dir + "/ml/ml_kfold_2to2_4day_night",
        dataset_files_gain_night[3],
        "1To1",
        "2To2",
        False,
        12,
    ...
    ml.main(
        steps_night,
        args.output_dir + "/ml/ml_kfold_2to2_3day_night",
        dataset_files_gain_night[4],
        "1To1",
        "2To2",
        False,
        12,
    ...
    ml.main(
        steps_night,
        args.output_dir + "/ml/ml_kfold_2to2_2day_night",
        dataset_files_gain_night[5],
        "1To1",
        "2To2",
        False,
        12,
    ...
    ml.main(
        steps_night,
        args.output_dir + "/ml/ml_kfold_2to2_1day_night",
        dataset_files_gain_night[6],
        "1To1",
        "2To2",
        False,
        12,
    ...
    ml.main(
        steps,
        args.output_dir + "/ml/ml_kfold_2to2_7day",
        dataset_files_gain[0],
        "1To1",
        "2To2",
        False,
        12,
    ...
    ml.main(
        steps,
        args.output_dir + "/ml/ml_kfold_2to2_6day",
        dataset_files_gain[1],
        "1To1",
        "2To2",
        False,
        12,
    ...
    ml.main(
        steps,
        args.output_dir + "/ml/ml_kfold_2to2_5day",
        dataset_files_gain[2],
        "1To1",
        "2To2",
        False,
        12,
    ...
    ml.main(
        steps,
        args.output_dir + "/ml/ml_kfold_2to2_4day",
        dataset_files_gain[3],
        "1To1",
        "2To2",
        False,
        12,
    ...
    ml.main(
        steps,
        args.output_dir + "/ml/ml_kfold_2to2_3day",
        dataset_files_gain[4],
        "1To1",
        "2To2",
        False,
        12,
    ...
    ml.main(
        steps,
        args.output_dir + "/ml/ml_kfold_2to2_2day",
        dataset_files_gain[5],
        "1To1",
        "2To2",
        False,
        12,
    ...
    ml.main(
        steps,
        args.output_dir + "/ml/ml_kfold_2to2_1day",
        dataset_files_gain[6],
        "1To1",
        "2To2",
        False,
        12,
