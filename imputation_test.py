...
if __name__ == "__main__":
    ...
    for WINDOW_ON, REMOVE_ZEROS, ANSCOMBE, LOG_ANSCOMBE in config:
        ...
        # _ = plt.hist(hist_array, density=True, bins=np.unique(hist_array).size)  # arguments are passed to np.histogram
        # plt.title("Histogram of raw input")
        # filename = args.output_dir + "/" + "histogram_raw_input_zeroson.png"
        # print(filename)
        # create_rec_dir(filename)
        ...
        # plt.cla()
        # fig, ax = plt.subplots()
        # hist_array = hist_array[hist_array != 0]
        # _ = plt.hist(hist_array, density=True, bins=np.unique(hist_array).size)  # arguments are passed to np.histogram
        # plt.title("Histogram of raw input")
        # filename = args.output_dir + "/" + "histogram_raw_input_zerosoff.png"
        # print(filename)
        # create_rec_dir(filename)
        # plt.savefig(filename)
