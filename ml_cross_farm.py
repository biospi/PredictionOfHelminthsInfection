...
def main(
    ...
    (
        ...
        find_dataset(str(farm1_path)), days, class_healthy, class_unhealthy
    )
    dataset2, _, _, _, _ = loadActivityData(
        find_dataset(str(farm2_path)), days, class_healthy, class_unhealthy
    )
    ...
    data_frame1 = df1_processed.loc[
        df1_processed["target"].isin([class_healthy_target, class_unhealthy_target])
    ]
    data_frame2 = df2_processed.loc[
        df2_processed["target"].isin([class_healthy_target, class_unhealthy_target])
    ]
    ...
    makeRocCurve(
        str(clf_best), output_dir, clf_best, X, y, None, slug, "Split", None, days
    )
